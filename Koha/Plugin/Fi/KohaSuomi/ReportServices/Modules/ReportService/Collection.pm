...
sub is_floating_by_float_rules {
    ...
    while(my($branches_key, $rule) = each %$rules) {
        ...
        if($item_homebranch =~ m/^$from_branch/ || $item_homebranch =~ m/^$to_branch/){
            $filtered_rules{$branches_key} = $rule;
        }
    }

    foreach my $current_branch (@$libraries){
        my %filtered_rules_by_library;
        my $checkrules;
        while(my($branches_key, $rule) = each %filtered_rules) {
            my ( $from_branch, $to_branch, $wildcard);
            $wildcard = $branches_key =~ m/%/ ? 1 : 0;
            ...
            if($current_branch =~ m/^$from_branch/ || $current_branch =~ m/^$to_branch/){
                $filtered_rules_by_library{$branches_key} = $rule;
                if($branches_key =~ m/->/){
                    if($wildcard){
        ...
        if ($item->barcode && scalar %filtered_rules_by_library > 0 && $checkrules){
            my $validate_floatrules = _validate_floatrules( $item, $biblioitem, \%filtered_rules_by_library, $current_branch );
...
sub _validate_floatrules {
    my ( $item, $biblioitem, $rules, $current_branch ) = @_;
